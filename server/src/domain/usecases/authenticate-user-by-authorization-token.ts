...
export class AuthenticateUserByAuthorizationToken {
  ...
  async authenticate(authorizationToken: string): Promise<AuthenticateUserByAuthorizationToken.Result> {
    try {
      const decrypted = await this.decrypter.decrypt(authorizationToken) as any
